...
- **`/api/server-stats` now reports the RAM-disk spool** (`bodies.spool`, TRDD-ZW4APOPI): dir,
  ...
  losing bodies) needed a `df` on the box to notice. `null` only when no spool is configured;
  `exists` says whether the path is there and `ownVolume` whether it is a real mount (a
  different device from the data dir) — a plain directory left on the boot volume after the RAM
  disk went away reports `exists: true, ownVolume: false` instead of the SSD's free space.
