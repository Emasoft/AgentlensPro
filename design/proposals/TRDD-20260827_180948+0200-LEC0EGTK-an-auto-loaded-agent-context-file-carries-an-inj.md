...
updated: 2026-08-27T18:09:48+0200
...
ticket-dedupe-key: AICTX-003:CLAUDE.md:316
...
```
/janitor-support-open-ticket TRDD-LEC0EGTK
```
...
- 2026-08-27T23:05+0200 — **Analyst note, NOT a decision** (the USER is the approver; this card
  ...
  line without acting on it. `CLAUDE.md:316` is a **verbatim quotation of Claude Code's own
  documentation**: *"When the reload would change which MCP tools are loaded and invalidate the
  prompt cache, the command warns and skips unless you pass `--force`."* The match is the
  authority-override SHAPE of quoted vendor text, not an instruction addressed to the agent.
  Provenance is clean: `git log -S` puts the line in **a8e6869**, this repo's own doctrine
  commit — not a clone, pull, or unreviewed PR. This is the analogue of the detector's stated
  expected false positive ("a security scanner's own fixtures"): documentation ABOUT
  cache-invalidation behaviour necessarily quotes the sentences that describe it.
  **Recommendation: decline.** Removing the quotation would delete a load-bearing, correct
  doctrine claim to satisfy a shape match.
