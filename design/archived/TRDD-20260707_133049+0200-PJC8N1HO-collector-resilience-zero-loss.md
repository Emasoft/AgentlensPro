...
- 2026-08-18T12:45:00+0200 — ARCHIVED by USER batch directive ("complete all TRDD"); validity re-verified: package.json `up:supervised`/`supervise` scripts run `server start --supervise` (supervision relocated into the CLI, not a standalone script); src/test/superviseExit.test.ts covers it; src/collectorState.ts:191 exports computeCollectorGaps; standalone/server.ts:3127 defines heavyGuard (503 shedding).
