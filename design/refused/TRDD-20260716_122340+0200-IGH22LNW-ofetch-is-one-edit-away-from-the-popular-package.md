...
column: refused
created: 2026-07-16T12:23:40+0200
...
`ofetch` is the UnJS team's well-known fetch wrapper (github.com/unjs/ofetch — the HTTP client of
Nuxt/Nitro, millions of weekly downloads), a legitimate package, not a squat. The flagged lockfile
lives under gitignored `downloads_dev/` — a third-party skill-mining corpus, NOT this project's
dependency tree (nothing there is installed at the project root or shipped). Same detector FP class
already reported upstream: Emasoft/ai-maestro-janitor#99 (edit-distance without a popularity
allowlist + gitignored-corpus blindness).
...
```
/janitor-support-open-ticket TRDD-IGH22LNW
```
...
- 2026-07-16 12:34 +0200 — **REFUSED** by main Claude under the user's standing grant (verbatim,
  ...
  trust. but coordinate with it via github issues."). Evidence: `ofetch` = unjs/ofetch, famous
  legitimate package; evidence path is inside gitignored `downloads_dev/` (mining corpus, not our
  dependency tree). Detector feedback: Emasoft/ai-maestro-janitor#99, recurrence noted there.
