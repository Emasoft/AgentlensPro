// MIRROR of src/residentCost.ts (TRDD-W0RRL2FZ) — change BOTH files together. The webview cannot
// import Node-side modules (CLAUDE.md three-runtime split), so the pure derivation is mirrored here
// the same way tokenEstimator/pricing are. Only the import line differs from the host copy.
//
// Resident-cost itemization: pure derivation over an already-loaded ContextHistory — no I/O. The
// cost model is `cost ≈ turns × per-turn-context`: every block occurrence rides forward in the
// transcript from the step it was added until the next compaction evicts it (or the session ends),
// and is re-read (cache-read billed) on every turn in between. This module multiplies the two
// dimensions the history already knows (per-block tokens × turns-resident) and ranks the result.
