...
suite('cacheBreakTimeline — buildCacheBreakTimeline (disk scan + previous_message_id chain)', () => {
  ...
  test('a hook that mutates every turn becomes ONE flagged SYSTEMATIC repeat-offender', async () => {
    ...
    const report = await buildCacheBreakTimeline({ bodiesDir: dir, sessionId: sid, minTokens: 5000, storeDir: noStore, hookEventsDir: noStore })
  ...
  test('coverage reports honest scan bounds and an absent directory never throws', async () => {
    ...
    const report = await buildCacheBreakTimeline({ bodiesDir: missing, storeDir: noStore, hookEventsDir: noStore })
...
suite('cacheBreakTimeline — buildCauseCostPeakReport (cross-session cause cost-peak)', () => {
  test('aggregates break events by CAUSE across ALL sessions in the scan, not just one target', async () => {
    ...
    const report = await buildCauseCostPeakReport({ bodiesDir: dir, minTokens: 5000, storeDir: noStore, hookEventsDir: noStore })
  ...
  test('bucket=output surfaces an OUTPUT-token-spike cause even when cache_creation ranks a different cause first', async () => {
    ...
    const byCache = await buildCauseCostPeakReport({ bodiesDir: dir, minTokens: 5000, storeDir: noStore, hookEventsDir: noStore })
    assert.strictEqual(byCache.groups[0].key, 'HOOK_INJECTION', '50000 cache_creation beats 1000 on the default bucket')

    const byOutput = await buildCauseCostPeakReport({ bodiesDir: dir, minTokens: 5000, bucket: 'output', storeDir: noStore, hookEventsDir: noStore })
  ...
  test('an absent bodies directory returns an empty report, never throwing', async () => {
    ...
    const report = await buildCauseCostPeakReport({ bodiesDir: missing, storeDir: noStore, hookEventsDir: noStore })
    assert.strictEqual(report.groupBy, 'cause')
...
suite('cacheBreakTimeline — agent-* child sessions resolve via the subagents transcript', () => {
  ...
  test('a child timeline classifies the CHILD stream carved out of the parent bucket (head recovered by fingerprint)', async () => {
    ...
    const report = await buildCacheBreakTimeline({ bodiesDir: bodies, sessionId: `agent-${CHILD_ID}`, minTokens: 5000, projectsDirs: [projects], storeDir: noStore, hookEventsDir: noStore })
    ...
    const bare = await buildCacheBreakTimeline({ bodiesDir: bodies, sessionId: CHILD_ID, minTokens: 5000, projectsDirs: [projects], storeDir: noStore, hookEventsDir: noStore })
  ...
  test('an agent-* id with no transcript returns an HONEST empty report naming the resolution path', async () => {
    ...
    const report = await buildCacheBreakTimeline({ bodiesDir: bodies, sessionId: 'agent-ffffffffffffffff0', minTokens: 5000, projectsDirs: [projects], storeDir: noStore, hookEventsDir: noStore })
