...
**And it must never be SLOW, which is harder than it looks.** With the server unreachable in a way
that HANGS rather than refuses (a firewall DROP, a suspended container, a VPN flap), the wrapper took
**10.6 seconds per render**. `AbortSignal.timeout` was not the problem — it fired correctly, the
fetch rejecting in 704 ms — but aborting a fetch does not destroy the underlying TCP socket, that
socket holds the event loop open until the OS connect timeout, and a CLI ending with
`process.exitCode` waits for the drain. **A timeout bounds the REQUEST, never the PROCESS.** The
hot-path commands (`statusline`, `hook`, `gate`) therefore `process.exit()` outright. Test this with
an address that **DROPS**: a dead port refuses instantly and hides the bug completely, which is
exactly why the existing end-to-end tests (pointed at `127.0.0.1:1`) never saw it.[^12]

**That fix has a second half, and forgetting it is a worse bug than the one it cured.**
`process.exit()` **discards** a queued piped stdout write past the ~64 KiB pipe buffer — measured,
262,144 bytes written and 65,536 received. `gate` writes the verdict Claude Code reads to decide
whether to block a tool call, so truncating it corrupts a **safety decision**, and small verdicts fit
in one buffer and would hide it forever. So: flush via a zero-length write's callback, **and bound
that wait**, because a reader that never drains restores the very hang the exit was added for. The
two constraints pull in opposite directions; assert BOTH, since a test that passes against either
mistake guards neither.[^13]
...
^ATOM-IXQ7-DOMU [desc:"The two query views that answer cost questions from the payload — cache (warm vs cold, cost bracketed by TTL tier) and peaks (delta + its gap) — and the measured agreement that validates the pricing t", keywords: was_that_turn_a_cache_miss verify_a_cache_miss_claim statusline-history_cache_view cache_write_vs_cache_read_per_turn is_the_1h_write_tier_real d_cost_across_an_idle_gap peaks_column_overstates_a_turn, ocd: 2026-08-01, lmd: 2026-08-01] [^5] [^6] [^7] [^8] [^9] [^10] [^11] [^12] [^13]
...
^ATOM-6BGQ-GK2M [desc: "The statusline capture reads Claude Code's JSON from our own stdin, concurrently with the inner command which gets a copy; stdout is inherited, so the inner script cannot affect our data — only its ex", keywords: will_fixing_the_inner_statusline_script_break_our_data_collection is_agentlenspro_affected_if_statusline.py_changes who_writes_statusline-debug.log does_the_wrapper_log_the_inner_command_output statusline_ingestion_independent_of_the_inner_command --inner_stdout_inherited_not_piped statusline_capture_reads_its_own_stdin a_huge_log_file_in_/tmp/claude_is_it_ours gating_another_tool's_debug_log_safe_for_us statusline_payload_copy_to_child, ocd: 2026-08-29, lmd: 2026-08-29]

The statusline wrapper ingests the JSON payload from ITS OWN stdin and is INDEPENDENT of the --inner command it wraps. In src/cli/statuslineCapture.ts (~189-194): payload = await readStdin(stdin); captureStatusline(payload, ...) and runInner(inner, payload) both START before either is awaited, so the capture rides ALONGSIDE the child rather than downstream of it. The inner script receives a COPY of the same payload on its stdin, and its stdout is INHERITED (it writes straight to our fd, no pipe, no copy) — so nothing the inner command prints, logs, or writes to disk can affect, delay, or corrupt what we capture. The ONLY coupling is the exit code, which we return as our own because a non-zero exit blanks Claude Code's status line. CONSEQUENCE for cross-tool debugging: if another tool owns the inner script, changing or silencing that script (e.g. gating a debug log) cannot break agentlenspro's statusline ingestion — the two read the same input independently. Ruling a foreign log file IN or OUT as ours is therefore structural, not empirical: inheritance means the bytes never pass through us. The residual risk is timing only: the capture is bounded by Promise.race(captured, residualCaptureMs()), so a slow inner child degrades the render, not the data.


^ATOM-SKNW-E15W [desc: "CC 2.1.252 prompt_cache block: authoritative per-turn cache verdict (ttl, expires_at, misses) captured in the statusline store; guaranteed columns, consumers, and the two historical data-quality cavea", keywords: prompt_cache_fields cache_expired_from_statusline expires_at_epoch_seconds harness_reports_the_cache_ttl is_the_ttl_5m_or_1h cache-expired_without_server stale_rate_limits_after_reset pre-2.1.243_rate_limit_percentage_wrong spend_limit_column_missing prompt_cache_warm prompt_cache_misses 5m_1h_cost_bracket_resolved, ocd: 2026-09-01, lmd: 2026-09-01]

Since Claude Code 2.1.252 the statusline payload carries the harness's OWN cache verdict as a prompt_cache block: warm, caching_observed, ttl ('5m'/'1h'), expires_at (epoch SECONDS, ts is ms), requests, misses, expected_rebuilds, hit_ratio, cache_write_tokens, miss_recache_tokens, last_miss_at (nullable), recache_tokens_if_cold. All 12 flattened prompt_cache_* columns are in GUARANTEED_COLUMNS (pre-2.1.252 samples lack the whole block — the trap-5 shape). Consumers: cache-expired answers from expires_at off disk with no server and no inference; the cache view resolves its 5m/1h cost bracket to ONE figure from prompt_cache_ttl and shows the harness misses counter. Field names were taken from a LIVE captured row (2026-09-01), never the docs. Two data-quality caveats for historical queries: samples written by CC < 2.1.243 can carry a STALE pre-reset rate_limits percentage after an idle window reset (harness bug, fixed 2.1.243), and rate_limits.spend_limit (2.1.251, gateway-only) is deliberately NOT guaranteed until a real captured row shows its flattened shape.
...
[^12]: [id:ATOM-XA98-PXFM, status:valid, keywords:"status_line_freezes_for_ten_seconds hook_is_slow_when_the_server_is_down AbortSignal_timeout_did_not_help aborted_fetch_keeps_the_process_alive dead_port_refuses_instantly_and_hides_the_bug process_exitCode_waits_for_the_event_loop", ocd:2026-08-01, lmd:2026-08-01] DO NOT assume `AbortSignal.timeout` bounds how long a CLI takes when a server is unreachable, BECAUSE aborting a fetch does not destroy the underlying TCP socket: the socket holds the event loop open until the OS connect timeout, and a CLI that ends by setting `process.exitCode` waits for that drain — measured 10.6 s per render on the status line, while the fetch itself correctly rejected in 704 ms. The timeout bounds the REQUEST, never the PROCESS. DO `process.exit()` on latency-critical commands, and test against an address that DROPS — a dead port refuses instantly and hides this completely.
[^13]: [id:ATOM-0VYJ-4IW6, status:valid, keywords:"process_exit_truncated_stdout gate_verdict_cut_off_at_65536_bytes piped_write_lost_on_exit fixing_the_hang_broke_the_output fast_exit_and_complete_output_pull_opposite_ways", ocd:2026-08-01, lmd:2026-08-01] DO NOT `process.exit()` to bound a command's runtime without flushing stdout first, BECAUSE exit DISCARDS a queued piped write past the ~64 KiB pipe buffer — measured, 262,144 bytes written and 65,536 received — and `gate` writes the verdict Claude Code reads to block a tool call, so the truncation corrupts a safety decision while small verdicts hide it forever. DO flush via a zero-length write's callback AND bound that wait, since a reader that never drains restores the very hang the exit was added for; then test BOTH failure directions, because a test passing against either mistake guards neither.
