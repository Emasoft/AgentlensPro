...
`gap_s` column is the working shape) so a consumer can label an interval total instead of guessing. [^1]

## Notes and lessons learned

[^1]: [id:ATOM-3JIQ-T518, status:valid, desc:"the memory-scope-leak detector flags this page as a high-entropy secret; verified 2026-08-05 to be our own camelCase identifiers", keywords:"memory-scope-leak_flags_this_page high-entropy_secret_false_positive is_there_a_secret_in_project_memory detector_says_demote_to_LOCAL_but_page_is_clean camelCase_identifier_looks_like_a_secret memgrep_keywords_underscore_phrases_flagged", ocd:2026-08-05, lmd:2026-08-05] DO NOT act on this page's `memory-scope-leak` finding by demoting it to LOCAL, BECAUSE it is a FALSE POSITIVE and the page is portable: verified 2026-08-05 with an exhaustive scan — zero home paths, usernames, emails or hostnames, and zero secret shapes (`sk-`, `ghp_`, `Bearer`, base64 blobs, uuids). The only strings long enough to trip a high-entropy heuristic are OUR OWN camelCase identifiers (`statuslineCostUsd`, `calcTokenCostUsd`) and kebab-case wiki-link slugs. DO verify the specific match before moving anything, and note that this is now the THIRD PROJECT page carrying the same false positive — `agentlenspro-ops-lessons` records the same verdict for the word "hostname" appearing inside the scope-routing rule that page QUOTES. Two structural sources, both inherent to the memory FORMAT: a page that explains the leak rule contains the rule's own trigger words, and every atom's mandatory `keywords:` field is underscore_joined English that scans as high-entropy. The detector lives in the janitor plugin, a DIFFERENT project — so the fix there is an issue filed upstream, never an edit from here.
