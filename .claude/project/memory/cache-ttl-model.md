...
^ATOM-R6DJ-YQEV [desc: "CC 2.1.257 version-gated cache-miss causes (advisor-set background requests uncached, RC Bash tool-def re-send, image-cap overflow) + /fork now cache-safe + fable-5-1 rates and the prefix-match pricin", keywords: advisor_model_compaction_uncached every_compaction_re-sent_full_conversation background_request_cache_miss_advisor remote_control_bash_tool_def_re-sent cache_miss_when_images_exceed_cap screenshot_heavy_session_misses_every_turn fork_keeps_prompt_cache_2.1.257 fable_5.1_pricing_cache_read_0.25 claude-fable-5-1_rates prefix_match_wrong_model_rates spurious_cache_warning_after_rewind, trdd: TRDD-SIGBCMGL, ocd: 2026-09-01, lmd: 2026-09-01]

CC 2.1.257 (2026-09-01) fixed four more cache-miss causes, all version-gated for attribution over older captures: (a) a session with an ADVISOR model set missed the prompt cache on every BACKGROUND request — compaction, /recap, prompt suggestions re-sent the full conversation uncached each time (a large hidden burn on advisor-enabled sessions before 2.1.257); (b) Remote Control connecting mid-session re-sent the Bash tool definition → one full-prefix miss; (c) screenshot-heavy sessions missed the cache on EVERY turn once images exceeded the per-request size cap; (d) /model and /effort showed a spurious cache warning after a rewind to empty. Also cache-SAFE from 2.1.257: /fork keeps the original conversation's prompt cache (its worktree briefing arrives as a message, not a system-prompt change). New default Fable is claude-fable-5-1 ($10/$50, $0.25 cache read — atypical 2.5% ratio; pricing.ts has an explicit key because the prefix matcher would otherwise bill it at fable-5's $1.00).
